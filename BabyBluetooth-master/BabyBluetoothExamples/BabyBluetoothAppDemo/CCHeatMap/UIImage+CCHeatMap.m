...
static inline void colorForValue32(float value, float *red, float *green, float *blue, float *alpha) {
...
static inline void colorForValue64(double value, double *red, double *green, double *blue, double *alpha) {
    static int maxVal = 255;
    if (value > 1) {
        value = 1;
    }
...
+ (UIImage *)heatMapWithRect:(CGRect)rect
    ...
    CGFloat weightSensitivity = 1; // Percents from maximum weight
    CGFloat weightBoostTo = 50;    // Percents to boost least sensible weight to
    
    // Adjustment variables for grouping
    NSInteger groupingThreshold = 10; // Increasing this will improve performance with
    // less accuracy. Negative will disable grouping
    int peaksRemovalThreshold = 20; // Should be greater than groupingThreshold
    CGFloat peaksRemovalFactor = 0.4; // Should be from 0 (no peaks removal) to 1
    // (peaks are completely lowered to zero)
    // Validate arguments
    ...
    // accessing NSArray objects)
    NSInteger points_num = (int)[points count];
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++, i++) {
            if (density[i] > 0) {
                ...
                if (sizeof(CGFloat) == 4) {
                    colorForValue32(floatDensity, &red, &green, &blue, &alpha);
                } else {
                    colorForValue64(floatDensity, &red, &green, &blue, &alpha);
                }
