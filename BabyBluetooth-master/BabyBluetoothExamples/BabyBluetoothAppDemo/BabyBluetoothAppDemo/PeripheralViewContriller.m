...
//    [self readPlantAssistantData];
    ...
    WriteValueViewController *writeVc = [[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"WriteValueViewController"];
...
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"断开链接" message:@"" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:nil];
//        [alert show];
//    }];
    //设置设备断开连接的委托
...
//    baby.having(self.currPeripheral).and.channel(channelOnPeropheralView).then.connectToPeripherals().begin();
//    [baby setBlockOnConnected:^(CBCentralManager *central, CBPeripheral *peripheral) {
//        NSLog(@"%@", peripheral);
//    }];
//    baby.enjoy();
//    baby.having(self.currPeripheral).and.channel(channelOnPeropheralView).then.connectToPeripherals().begin();
    baby.having(self.currPeripheral).and.connectToPeripherals().begin();
